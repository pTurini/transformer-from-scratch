...
class FeedForward(nn.Module):
    def __init__(self, n_embd):
        ...
        self.net = nn.Sequential(nn.Linear(n_embd, 4 * n_embd), 
                                 ...
                                 nn.Dropout(dropout),
                                 ) #creates Linear NN and RELu activation in sequence
...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.sa(self.ln1(x)) #skip connections, layer norm
        x = x + self.ffwd(self.ln2(x)) #skip connections, layer norm
        return x
